...
births_weeks_temp <- births_weeks_temp |> 
  select(id, clim_zone, tbw, trim_gest, sex,  
...
for (i in seq_along(clim_zones)) {
  ...
  pred_data$predicted_tbw <- predict(gam0, newdata = pred_data, se.fit = TRUE)$fit
  pred_data$se <- predict(gam0, newdata = pred_data, se.fit = TRUE)$se.fit
  pred_data <- pred_data |> 
    mutate(lower_ci = predicted_tbw - 1.96 * se,
           upper_ci = predicted_tbw + 1.96 * se)
  
  # Generar el gráfico
  tags <- LETTERS[1:length(clim_zones)]
  
  p <- ggplot(pred_data, aes(x = temp_mean_gest, y = predicted_tbw)) +
    ...
    labs(title = paste(zone),
         ...
         y = "Predicted estimated tBW and 95% CI (grams)",
         tag = paste0(tags[i], ".")) +
    theme_bw() +
...
ggsave(filename = paste0(data_out, "fig/", "Unadjusted_GAM_models_tbw_mean", ".png"),
       res = 300,
       width = 32,
       height = 16,
       units = 'cm',
...
for (i in seq_along(clim_zones)) {
  ...
  p <- ggplot(pred_data, aes(x = temp_min_gest, y = predicted_tbw)) +
    geom_line(color = "black", size = 1) +
    geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "lightblue", alpha = 0.5) +
    #scale_y_continuous(limits = c(3000, 3800)) +
    #scale_x_continuous(n.breaks = 5) +
...
# Add combined plot 
gam0 <- gam(tbw ~ s(temp_min_percentile100_cz),
            data = births_weeks_temp_wide, 
            family=gaussian()) 
...
gam_plots[["Combined"]] <- ggplot(combined_pred_data, aes(x = temp_min_percentile100_cz, y = predicted_tbw)) +
  geom_line(color = "black", size = 1) +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "lightblue", alpha = 0.5) +
  #scale_y_continuous(limits = c(3000, 3800)) +
  labs(title = "Combined, all climatic zones percentiles",
...
ggsave(filename = paste0(data_out, "fig/", "Unadjusted_GAM_models_tbw_min", ".png"),
       res = 300,
       width = 32,
       height = 16,
       units = 'cm',
...
for (i in seq_along(clim_zones)) {
  ...
  p <- ggplot(pred_data, aes(x = temp_max_gest, y = predicted_tbw)) +
    geom_line(color = "black", size = 1) +
    geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "lightblue", alpha = 0.5) +
    #scale_y_continuous(limits = c(3000, 3800)) +
    #scale_x_continuous(n.breaks = 5) +
...
# Add combined plot 
gam0 <- gam(tbw ~ s(temp_max_percentile100_cz),
            data = births_weeks_temp_wide, 
            family=gaussian()) 
...
gam_plots[["Combined"]] <- ggplot(combined_pred_data, aes(x = temp_max_percentile100_cz, y = predicted_tbw)) +
  geom_line(color = "black", size = 1) +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "lightblue", alpha = 0.5) +
  #scale_y_continuous(limits = c(3000, 3800)) +
  labs(title = "Combined, all climatic zones percentiles",
...
ggsave(filename = paste0(data_out, "fig/", "Unadjusted_GAM_models_tbw_max", ".png"),
       res = 300,
       width = 32,
       height = 16,
       units = 'cm',
...
gam0_1 <- gam(tbw ~ temp_mean_percentile_wcz,
            data = births_weeks_temp, 
            family=gaussian()) 

gam0_2 <- gam(tbw ~ temp_mean_percentile_tcz_T1 + temp_mean_percentile_tcz_T2 + temp_mean_percentile_tcz_T3,
            data = births_weeks_temp_wide, 
            family=gaussian()) 

# Adjusted
gam1 <- gam(tbw ~ temp_mean_percentile_wcz + sex + 
              age_group_mom + educ_group_mom + job_group_mom +
              age_group_dad + educ_group_dad + job_group_dad +
...
gam2 <- gam(tbw ~ temp_mean_percentile_tcz_T1 + temp_mean_percentile_tcz_T2 + temp_mean_percentile_tcz_T3+
              sex + 
              age_group_mom + educ_group_mom + job_group_mom +
              age_group_dad + educ_group_dad + job_group_dad +
              s(year_week1) + s(month_week1),  
...
# Save results 
m0_1 <-  broom::tidy(gam0_1, parametric = TRUE, conf.int = TRUE, conf.level = 0.95) |> mutate_if(is.numeric, round, 2)
m0_2 <-  broom::tidy(gam0_2, parametric = TRUE, conf.int = TRUE, conf.level = 0.95) |> mutate_if(is.numeric, round, 2)
m1 <-  broom::tidy(gam1, parametric = TRUE, conf.int = TRUE, conf.level = 0.95) |> mutate_if(is.numeric, round, 2)
m2 <-  broom::tidy(gam2, parametric = TRUE, conf.int = TRUE, conf.level = 0.95) |> mutate_if(is.numeric, round, 2)
...
write.xlsx(tables_list, file =  paste0(data_out, "tab/", "GAM_models_tbw_mean", ".xlsx"))
...
ggsave(filename = paste0(data_out, "fig/", "Adjusted_GAM_models_tbw_trim_mean", ".png"),
       res = 300,
       width = 28,
       height = 12,
       units = 'cm',
...
gam0_1 <- gam(tbw ~ temp_min_percentile_wcz,
              data = births_weeks_temp, 
              family=gaussian()) 

gam0_2 <- gam(tbw ~ temp_min_percentile_tcz_T1 + temp_min_percentile_tcz_T2 + temp_min_percentile_tcz_T3,
              data = births_weeks_temp_wide, 
              family=gaussian()) 

# Adjusted
gam1 <- gam(tbw ~ temp_min_percentile_wcz + sex + 
              age_group_mom + educ_group_mom + job_group_mom +
              age_group_dad + educ_group_dad + job_group_dad +
              s(year_week1) + s(month_week1),  
            data = births_weeks_temp, 
...
gam2 <- gam(tbw ~ temp_min_percentile_tcz_T1 + temp_min_percentile_tcz_T2 + temp_min_percentile_tcz_T3+
              sex + 
              age_group_mom + educ_group_mom + job_group_mom +
              age_group_dad + educ_group_dad + job_group_dad +
              s(year_week1) + s(month_week1),  
...
write.xlsx(tables_list, file =  paste0(data_out, "tab/", "GAM_models_tbw_min", ".xlsx"))
...
ggsave(filename = paste0(data_out, "fig/", "Adjusted_GAM_models_tbw_trim_min", ".png"),
       res = 300,
       width = 28,
       height = 12,
       units = 'cm',
...
gam0_1 <- gam(tbw ~ temp_max_percentile_wcz,
              data = births_weeks_temp, 
              family=gaussian()) 

gam0_2 <- gam(tbw ~ temp_max_percentile_tcz_T1 + temp_max_percentile_tcz_T2 + temp_max_percentile_tcz_T3,
              data = births_weeks_temp_wide, 
              family=gaussian()) 

# Adjusted
gam1 <- gam(tbw ~ temp_max_percentile_wcz + sex + 
              age_group_mom + educ_group_mom + job_group_mom +
              age_group_dad + educ_group_dad + job_group_dad +
              s(year_week1) + s(month_week1),  
            data = births_weeks_temp, 
...
gam2 <- gam(tbw ~ temp_max_percentile_tcz_T1 + temp_max_percentile_tcz_T2 + temp_max_percentile_tcz_T3+
              sex + 
              age_group_mom + educ_group_mom + job_group_mom +
              age_group_dad + educ_group_dad + job_group_dad +
              s(year_week1) + s(month_week1),  
...
write.xlsx(tables_list, file =  paste0(data_out, "tab/", "GAM_models_tbw_max", ".xlsx"))
...
ggsave(filename = paste0(data_out, "fig/", "Adjusted_GAM_models_tbw_trim_max", ".png"),
       res = 300,
       width = 28,
       height = 12,
       units = 'cm',
...
ggsave(filename = paste0(data_out, "fig/", "Adjusted_GAM_models_tbw_trim_full", ".png"),
       res = 300,
       width = 28,
       height = 30,
       units = 'cm',
