...
births_weeks_temp <- births_weeks_temp %>% 
  ...
  mutate(clim_zone=factor(clim_zone, 
                     ...
                     labels=c("Desert hot", "Desert cold", "Temperate dry, hot summer", "Temperate dry, warm summer", "Temperate, no dry season", "Cold steppe")
  )) %>% 
  ...
  mutate(date_week1 = (date_start - (7 * abs(week_gest_num - row_number()))) - weeks(1), 
  ...
  relocate(age_group_dad, educ_group_dad, job_group_dad, .after=job_dad) %>% 
    ...
    ungroup() %>% 
    group_by(zone, week_gest_num) %>% 
    mutate(temp_mean_percentile_wz = ntile(temp_mean, 10)) %>% 
    ...
    ungroup() %>% 
    group_by(zone, week_gest_num) %>% 
    mutate(temp_min_percentile_wz = ntile(temp_min, 10)) %>% 
    group_by(clim_zone, week_gest_num) %>% 
    ...
    ungroup() %>% 
    group_by(zone, week_gest_num) %>% 
    mutate(temp_max_percentile_wz = ntile(temp_max, 10)) %>% 
    group_by(clim_zone, week_gest_num) %>% 
...
births_weeks_temp <- 
  ...
  mutate(vulnerability = case_when(
    ...
    vulnerability=="Medio-baja" ~ "Medium low",
    vulnerability=="Medio-alta" ~ "Medium high",
...
births_weeks_hum <- births_weeks_hum %>% 
  ...
  ungroup() %>% 
  group_by(zone, week_gest_num) %>% 
  mutate(hum_mean_percentile_wz = ntile(hum_mean, 10)) %>% 
  ungroup() %>% 
  group_by(week_gest_num) %>% 
  mutate(hum_min_percentile_w = ntile(hum_min, 10)) %>% 
  ungroup() %>% 
  group_by(zone, week_gest_num) %>% 
  mutate(hum_min_percentile_wz = ntile(hum_min, 10)) %>% 
  ungroup() %>% 
  group_by(week_gest_num) %>% 
  mutate(hum_max_percentile_w = ntile(hum_max, 10)) %>% 
  ungroup() %>% 
  group_by(zone, week_gest_num) %>% 
  mutate(hum_max_percentile_wz = ntile(hum_max, 10)) %>% 
  ungroup() %>% 
  mutate(across(contains("percentile"), ~factor(case_when(
    . == 1 ~ "<=10", 
    . == 2 ~ "11-20",
    . == 3 ~ "21-30",
    . == 4 ~ "31-40",
